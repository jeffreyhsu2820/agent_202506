...
class GetCurrentTimeInput(BaseModel):
    timezone: str = Field(
        ...,
        description="IANA timezone name (e.g., 'Asia/Taipei', 'America/NewYork', 'Europe/London')"
    )

# Output model for time results
class TimeResult(BaseModel):
    timezone: str = Field(..., description="The timezone used for the time calculation")
    datetime: str = Field(..., description="The current datetime in ISO format (e.g., '2025-04-23T15:30:00')")
    is_dst: bool = Field(..., description="Indicates if daylight saving time is in effect")
